    ...
    if st.button("Start Detection"):
        ...
        while cap.isOpened():
            ...
            if face_results.detections:
                detection = face_results.detections[0]  # Only track the first face detected
                ...
                if face_crop.size > 0:
                    ...
                    cv2.putText(raw, label, (5, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
                    cv2.putText(cam, label, (5, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
                    yawn_disp.image([raw, cam], caption=["Yawn Input", "Yawn Grad-CAM"], width=200)
            ...
            if mesh_results.multi_face_landmarks:
                ...
                for i, eye_pts in enumerate([LEFT_EYE, RIGHT_EYE]):
                    ...
                    if eye_crop.size > 0:
                        ...
                        cv2.putText(cam, label, (5, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
                        if i == 0:
                            left_eye_disp.image([raw, cam], caption=["Left Eye", "Grad-CAM"], width=180)
                        else:
                            right_eye_disp.image([raw, cam], caption=["Right Eye", "Grad-CAM"], width=180)
